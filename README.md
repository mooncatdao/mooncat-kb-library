...
The generated, gitignored `public/kb/` directory contains `manifest.json` and a copied `content/` tree. The generator includes only `README.md`, `llms.txt`, and supported Markdown, JSON, or text files beneath `docs/`, `data/`, and `examples/`. It omits hidden files, development metadata, `references/`, `scripts/`, `AGENTS.md`, `result.md`, and unsupported types. It records the source git commit when available but does not require git metadata.

## V1 limitations

- This is a static, hash-routed presentation layer; it does not edit, search, authenticate, or call a backend.
